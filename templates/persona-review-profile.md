...
repository and fill in every section. The `reviewed-writer` skills read the
eleven `##` headings below by name: keep their exact wording. Each body is prose
...
Repository-specific values for the `reviewed-writer` plugin's `write-doc` and
`persona-review` skills. The skills read this file first and refer to its
sections by name.
