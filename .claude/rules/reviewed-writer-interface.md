...
the `write-doc` and `persona-review` skills and the `diataxis-persona-engine`
and `persona-reviewer` agents — arrives through the pin in
`.claude/settings.json`, so reviews run with the pinned release, not the working
tree. The profile's `##` headings are read by name by the plugin's engine agent
and `persona-review` skill; the persona head files and the declaration file are
